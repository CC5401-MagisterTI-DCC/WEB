...
public class ValidationFilter implements Filter {
	...
	private static String ERROR_DUPLICADA = "/duplicada.jsp";
	...
	public void doFilter(ServletRequest req, ServletResponse res, FilterChain chain) throws IOException, ServletException {
		...
		PostulacionDAO postulacionDAO = PostulacionDAOFactory.getPostulacionDAO();
		...
		if(request.getMethod().equalsIgnoreCase("POST")){
			...
			try{
				...
				if(nombre == null || nombre.equals("") || !Validacion.validateName(nombre))
					throw new Exception("Error en el nombre " + nombre);

				String apellido = request.getParameter("apellido").trim();
				if(apellido == null || apellido.equals("") || !Validacion.validateName(apellido))
				...
				if(genero == null || genero.equals("") || (!genero.equalsIgnoreCase("Masculino") && !genero.equalsIgnoreCase("Femenino")))
					throw new Exception("Error en el genero");

				String fechaNacimiento = request.getParameter("fecha_nac").trim();
				Date convertedFechaNacimiento = sdf.parse(fechaNacimiento);

				String emailPersonal = request.getParameter("emailfield").trim();
				String emailPersonal2 = request.getParameter("emailfield2").trim();
				if(emailPersonal == null || emailPersonal2 == null || 
						!emailPersonal.equals(emailPersonal2) || emailPersonal.equals("") ||
				...
				if(fonoPersonal == null || fonoPersonal.equals("") || !Validacion.validatePhone(fonoPersonal))
				...
				String direccionResidencia = request.getParameter("direccion").trim();
				...
				if(tipoDoc == null || 
						(!tipoDoc.equalsIgnoreCase("rut") && 
								!tipoDoc.equalsIgnoreCase("pass")))
					throw new Exception("Error en tipo de documento");
				if(tipoDoc.equalsIgnoreCase("rut") && !Validacion.validateRut(request.getParameter("rut").trim()))
				...
				//TODO: ESPECIFICAR CASOS EN QUE UN POSTULANTE PUEDE VOLVER A POSTULAR
				PostulacionDTO postulacionDB = postulacionDAO.getPostulacionActiva(identificacion);
				if(postulacionDB != null){
					RequestDispatcher view = request.getRequestDispatcher(ERROR_DUPLICADA);
				...
				for(int i = 1; i<10;i++){
					if(request.getParameterMap().containsKey("grado" + i)){
						...
						sdf.parse(fechaObtencion);
						if(paisDAO.get(Integer.parseInt(paisGrado)) == null)
							throw new Exception("Error en pais de la institucion " + i);
						j++;
					}
				}

				//financiamiento
				String financiamiento= request.getParameter("financiamiento").trim();
				if(financiamiento == null || (!financiamiento.equalsIgnoreCase("Particular") &&
						!financiamiento.equalsIgnoreCase("Empresa") && !financiamiento.equalsIgnoreCase("Beca")))
					throw new Exception("Error en financiamiento");


				Part part = null;
				part = request.getPart("cert_titulo");
				if(part.getContentType() == null || !part.getContentType().equalsIgnoreCase("application/pdf") || part.getSize() > maxSize)
					throw new Exception("Error en Certificado de titulo 1");
				part = request.getPart("cert_notas");
				if(part.getContentType() == null || !part.getContentType().equalsIgnoreCase("application/pdf") || part.getSize() > maxSize)
					throw new Exception("Error en Certificado de notas titulo 1");
				for(int i = 1; i<j; i++){
					//Certificado de grado
					if(request.getParameterMap().containsKey("grado" + i)){
						part = request.getPart("cert_titulo" + i);
						if(part.getContentType() == null || !part.getContentType().equalsIgnoreCase("application/pdf") || part.getSize() > maxSize)
							throw new Exception("Error en Certificado de titulo " + (i+1));
						part = request.getPart("cert_notas" + i);
						if(part.getContentType() == null || !part.getContentType().equalsIgnoreCase("application/pdf") || part.getSize() > maxSize)
							throw new Exception("Error en Certificado de notas titulo " + (i+1));
					}
				}
				...
				if(part.getContentType() == null || !part.getContentType().equalsIgnoreCase("application/pdf") || part.getSize() > maxSize)
					throw new Exception("Error en Carta de Presentacion");
				part = request.getPart("carta_rec_1");
				if(part.getContentType() == null || !part.getContentType().equalsIgnoreCase("application/pdf") || part.getSize() > maxSize)
					throw new Exception("Error en Carta Recomendacion 1");
				part = request.getPart("carta_rec_2");
				if(part.getContentType() == null || !part.getContentType().equalsIgnoreCase("application/pdf") || part.getSize() > maxSize)
					throw new Exception("Error en Carta Recomendacion 2");
