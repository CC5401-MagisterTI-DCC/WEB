...
public class EditController extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try{
			...
			//identificacion
			IdentificacionDTO identificacion = identificacionDAO.get(idIdentificacion);
			...
			for(int i = 0; i<grado.length;i++){
				gradosAcademicos[i] = gradoAcademicoDAO.getFromId(Integer.parseInt(idGrado[i]));
			}
			...
			PostulacionDTO postulacion = postulacionDAO.getPostulacion(idPostulacion);
			...
			if(empresa!=null && !empresa.equals("")){
				...
				datosEmpresa = new DatosEmpresaDTO(idDatosEmpresa,postulante.getId(),empresa,cargo,direccionEmpresa,fonoEmpresa);
			...
			if(part.getContentType() != null && part.getContentType().equalsIgnoreCase("application/pdf")){
				...
				CP.setId(Integer.parseInt(request.getParameter("idCartaPresentacion")));
