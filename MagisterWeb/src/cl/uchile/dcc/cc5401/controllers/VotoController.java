	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(dl==null){
			...
			if(decision.equalsIgnoreCase("aceptado")){
				VotoDTO voto = new VotoDTO(0,id,user.getId(),TipoVoto.POSITIVO);
				//Verificamos si existía un voto anterior del usuario en el sistema. De ser así, se actualiza, si no, se crea un nuevo voto
				if(votoDAO.get(id, user.getId())!=null){
					votoDAO.actualizar(voto, id, user.getId());
					historialDAO.agregar(new HistorialDTO(0,postulacion.getId(),"<strong>"+user.getUsername()+"</strong>: Votación: Aceptado",new Date(),comentario));
				}
				else{
			...
			//Si fue un voto negativo
				...
				VotoDTO voto = new VotoDTO(0,id,user.getId(),TipoVoto.NEGATIVO);
				//Verificamos si existía un voto anterior del usuario en el sistema. De ser así, se actualiza, si no, se crea un nuevo voto
				if(votoDAO.get(id, user.getId())!=null){
					votoDAO.actualizar(voto, id, user.getId());
					historialDAO.agregar(new HistorialDTO(0,postulacion.getId(),"<strong>"+user.getUsername()+"</strong>: Votación: Rechazado",new Date(),comentario));
				}
				else{
					votoDAO.agregar(voto);
					historialDAO.agregar(new HistorialDTO(0,postulacion.getId(),"<strong>"+user.getUsername()+"</strong>: Votación: Rechazado",new Date(),comentario));
		...
		//Si es un request de deadline de votación, lo actualizamos
			...
			try {
				...
				forward = SUCCESS_PAGE;
				historialDAO.agregar(new HistorialDTO(0,postulacion.getId(),"<strong>"+user.getUsername()+"</strong>: Se actualiza deadline a "+request.getParameter("deadline"),new Date(),""));
