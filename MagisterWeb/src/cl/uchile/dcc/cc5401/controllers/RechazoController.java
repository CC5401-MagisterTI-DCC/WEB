...
public class RechazoController extends HttpServlet {
	...
	public void init(ServletConfig config) throws ServletException { 
        ...
        rechazoSubject = config.getServletContext().getInitParameter("rechazoSubject");
        rechazoBody = config.getServletContext().getInitParameter("rechazoBody"); 
        paginaRechazo = config.getServletContext().getInitParameter("paginaRechazo"); 
	}
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try{
			...
			PostulacionDTO postulacion = postulacionDAO.getPostulacion(Integer.parseInt(idPostulacion));
			...
			historialDAO.agregar(new HistorialDTO(0,postulacion.getId(),"<strong>"+user.getUsername()+"</strong>: <i class='icon-exclamation-sign'></i>  Rechazo de documentos",new Date(),""));
			postulacionDAO.actualizar(postulacion);

			
			//Enviamos el mail al postulante
			rechazoBodyP = rechazoBody.replaceAll("@nombre", postulacion.getNombrePostulante());
			rechazoBodyP = rechazoBodyP.replaceAll("@track", HashHelper.toHash(String.valueOf(postulacion.getId()*10+postulacion.getIdPostulante()), Algoritmo.MD5));
			rechazoBodyP = rechazoBodyP.replaceAll("@paginaRechazo", paginaRechazo);
			rechazoBodyP = rechazoBodyP.replaceAll("@razon", razon);
			try{
				mailHelper.sendMail(postulante.getEmail(), rechazoSubject, rechazoBodyP);
			}catch(Exception e){
				e.printStackTrace();
