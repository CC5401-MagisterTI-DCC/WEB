	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try{
			...
			try{
				...
				e.printStackTrace();
				System.out.println("Ocurrió un error al enviar mail a "+postulante.getEmail());
			}
			historialDAO.agregar(new HistorialDTO(0,postulacion.getId(),"Se ingresó la postulación al sistema",new Date(),""));
			forward=SUCCESS;
		}
		catch (Exception e){
			e.printStackTrace();
